...
const MoviesGenres = () => {
  ...
  const buttonsData = [
    {
      genre: '2',
      label: 'drama',
      icon: <FaTheaterMasks fontSize={'140px'} color="#fff" className="genres-icons" />,
    },
    {
      genre: '17',
      label: 'horror',
      icon: <FaGhost fontSize={'140px'} color="#fff" className="genres-icons" />,
    },
    {
      genre: '12',
      label: 'fantasy',
      icon: <FaDragon fontSize={'140px'} color="#fff" className="genres-icons" />,
    },
    {
      genre: '21',
      label: 'sport',
      icon: <FaBasketballBall fontSize={'140px'} color="#fff" className="genres-icons" />,
    },
    {
      genre: '16',
      label: 'music',
      icon: <FaMusic fontSize={'140px'} color="#fff" className="genres-icons" />,
    },
  ];
  ...
  const moviesRequest = (genre) => {
    ...
    getMovieGenres(genre).then(onMoviesLoaded);
  };

  const buttons = buttonsData.map(({ genre, label, icon }) => {
    const active = moviesGenre === genre;
    const clazz = active
      ? 'btn-reset movies-genres__btn movies-genres__btn-active'
      : 'btn-reset movies-genres__btn';

    return (
      <button type="button" className={clazz} key={genre} onClick={() => moviesRequest(genre)}>
        {icon}
  ...
  const renderItems = (arr) => {
          ...
          <div className="movie">
            ...
            <div className="movie__info">
              <h4 className="movie__title">
                {item.title.length > 18 ? item.title.substring(0, 19) + '...' : item.title}
              </h4>
    ...
    return <ul className="movies-genres__list list-reset">{items}</ul>;
  };
  ...
  return (
    <section className="movies-genres">
      <div className="container">
        ...
        <div className="movies-genres__btns">{buttons}</div>
        <div className="movies-genres__divider"></div>
        {items}
        {spinner}
      </div>
